...
const SCRIPT = `(async()=>{const allOut=[];let page=1;while(page<=20){let last=0;for(let i=0;i<8;i++){window.scrollTo(0,document.body.scrollHeight);await new Promise(r=>setTimeout(r,900));if(document.body.scrollHeight===last)break;last=document.body.scrollHeight;}await new Promise(r=>setTimeout(r,800));const imgs=[...document.querySelectorAll('#one_search img[src*="images.cdn.manheim.com"]')];const map=new Map();for(const img of imgs){let pp=img.closest('[data-vin], [data-testid*="listing"], article, li')||img.parentElement;let vin=null;for(let d=0;d<6&&pp;d++){const t=(pp.innerText||'')+(pp.dataset?JSON.stringify(pp.dataset):'');const m=t.match(/\b[A-HJ-NPR-Z0-9]{17}\b/);if(m){vin=m[0];break;}pp=pp.parentElement;}if(!vin)continue;if(!map.has(vin))map.set(vin,[]);if(!map.get(vin).includes(img.src))map.get(vin).push(img.src);}for(const[vin,images]of map)for(const image of images.slice(0,4))if(!allOut.some(o=>o.vin===vin&&o.image===image))allOut.push({vin,image});console.log('Página '+page+': '+map.size+' VINs, total '+allOut.length+' fotos');const next=[...document.querySelectorAll('button, a')].find(b=>{const t=(b.innerText||b.textContent||'').trim().toLowerCase();return (t==='next'||t==='siguiente'||t==='>'||t==='›'||b.getAttribute('aria-label')?.toLowerCase().includes('next'))&&!b.disabled&&b.offsetParent!==null;});if(!next||page>=20)break;next.click();await new Promise(r=>setTimeout(r,3500));page++;}if(allOut.length===0){const cards=[...document.querySelectorAll('#one_search img')].map(i=>i.src).filter(s=>s.includes('images.cdn.manheim.com')).slice(0,800);const vins=[...document.documentElement.innerHTML.matchAll(/\b[A-HJ-NPR-Z0-9]{17}\b/g)].map(m=>m[0]);const seen=new Set();const uniq=vins.filter(v=>{if(seen.has(v))return false;seen.add(v);return true;});uniq.forEach((v,idx)=>{for(let k=0;k<3;k++){const im=cards[idx*3+k];if(im)allOut.push({vin:v,image:im});}});}const b=new Blob([JSON.stringify(allOut,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='manheim-con-fotos.json';a.click();alert('✅ '+allOut.length+' fotos capturadas ('+new Set(allOut.map(o=>o.vin)).size+' VINs) en '+page+' páginas')})()`;
