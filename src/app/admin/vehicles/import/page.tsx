...
const SCRIPT = `(async()=>{const cards=[...document.querySelectorAll('#one_search img')].map(i=>i.src).filter(s=>s.includes('http')).slice(0,600);const vins=[...document.documentElement.innerHTML.matchAll(/\\b[A-HJ-NPR-Z0-9]{17}\\b/g)].map(m=>m[0]);const seen=new Set();const uniq=vins.filter(v=>{if(seen.has(v))return false;seen.add(v);return true;});const out=[];uniq.forEach((v,idx)=>{const img=cards[idx*2]||cards[idx]||'';if(img.includes('images.cdn.manheim.com')) out.push({vin:v,image:img});});if(out.length===0) vins.slice(0,50).forEach((v,i)=>cards[i]&&out.push({vin:v,image:cards[i]}));const b=new Blob([JSON.stringify(out,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='manheim-con-fotos.json';a.click();alert('✅ '+out.length+' fotos capturadas')})()`;
