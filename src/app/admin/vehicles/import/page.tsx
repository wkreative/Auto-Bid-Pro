...
function parseCSV(text: string): ParsedVehicle[] {
  ...
  return lines.slice(1).map(l => { const c = parseCSVLine(l, delim); const vin = (c[iVin] || '').replace(/[^A-Za-z0-9]/g, '').toUpperCase().slice(0, 17); const grade = c[iGrade]||''; const seller = c[iSeller]||''; const auction = c[iAuction]||''; const comments = c[iSellerComments]||''; const notes = c[iNotes]||''; const descParts = []; if(grade) descParts.push(`Condición: ${grade}/5`); if(seller) descParts.push(`Vendedor: ${seller}`); if(auction) descParts.push(`Casa: ${auction}`); if(comments) descParts.push(`Comentarios: ${comments}`); if(notes) descParts.push(`Notas: ${notes}`); return { brand: c[iMake] || 'N/A', model: c[iModel] || 'N/A', year: parseInt(c[iYear]) || 2020, vin, mileage: parseInt((c[iOdo] || '0').replace(/[^0-9]/g, '')) || 0, location: c[iLoc] || 'Puerto Rico - Manheim Caribbean', starting_price: parseFloat((c[iPrice] || '1000').replace(/[^0-9.]/g, '')) || 1000, images: [], trim: c[iTrim], exterior_color: c[iColor], description: descParts.join(' | ') }; }).filter(v => v.vin.length >= 5);
}

const SCRIPT = `(async()=>{const allOut=[];let page=1;const origFetch=window.fetch;const origOpen=XMLHttpRequest.prototype.open;const origSend=XMLHttpRequest.prototype.send;window._logs=[];window.fetch=async function(...a){const r=await origFetch.apply(this,a);const u=typeof a[0]==='string'?a[0]:a[0].url||'';if(u.includes('manheim')||u.includes('search')||u.includes('listing')){r.clone().json().then(j=>{const s=JSON.stringify(j);if(s.includes('vin')||s.includes('Vin')){window._logs.push({u,j});console.log('Capturado',u);}}).catch(()=>{});}return r;};XMLHttpRequest.prototype.open=function(m,u){this._u=u;return origOpen.apply(this,arguments);};XMLHttpRequest.prototype.send=function(...a){this.addEventListener('load',function(){try{const j=JSON.parse(this.responseText);if(JSON.stringify(j).includes('vin')){window._logs.push({u:this._u,j});console.log('XHR',this._u);}}catch(e){}});return origSend.apply(this,a);};while(page<=20){let last=0;for(let i=0;i<6;i++){window.scrollTo(0,document.body.scrollHeight);await new Promise(r=>setTimeout(r,900));if(document.body.scrollHeight===last)break;last=document.body.scrollHeight;}await new Promise(r=>setTimeout(r,900));const imgs=[...document.querySelectorAll('#one_search img[src*="images.cdn.manheim.com"]')];const mp=new Map();for(const im of imgs){let pa=im.closest('[data-vin], [data-testid*="listing"], article, li')||im.parentElement;let v=null;for(let d=0;d<6&&pa;d++){const txt=(pa.innerText||'')+(pa.dataset?JSON.stringify(pa.dataset):'');const m=txt.match(/\b[A-HJ-NPR-Z0-9]{17}\b/);if(m){v=m[0];break;}pa=pa.parentElement;}if(!v)continue;if(!mp.has(v))mp.set(v,[]);if(!mp.get(v).includes(im.src))mp.get(v).push(im.src);}for(const[v,ims]of mp)for(const im of ims.slice(0,4))if(!allOut.some(o=>o.vin===v&&o.image===im))allOut.push({vin:v,image:im});console.log('Pag '+page+': '+mp.size+' VINs total '+allOut.length);const nxt=[...document.querySelectorAll('button, a')].find(b=>{const tt=(b.innerText||b.textContent||'').trim().toLowerCase();return (tt==='next'||tt==='siguiente'||tt==='>'||tt==='›'||b.getAttribute('aria-label')?.toLowerCase().includes('next'))&&!b.disabled&&b.offsetParent!==null;});if(!nxt||page>=15)break;nxt.click();await new Promise(r=>setTimeout(r,3500));page++;}if(allOut.length===0&&window._logs.length>0){for(const lg of window._logs){let d=lg.j;let lst=[];if(Array.isArray(d))lst=d;else if(d.listings)lst=d.listings;else if(d.vehicles)lst=d.vehicles;else if(d.results)lst=d.results;else if(d.data)lst=Array.isArray(d.data)?d.data:[d.data];else lst=[d];for(const it of lst){const vin=(it.vin||it.Vin||it.VIN||'').toString().toUpperCase();if(!vin)continue;const ims=it.images||it.imageUrls||it.photos||[];const arr=Array.isArray(ims)?ims:[ims];for(const im of arr.filter(Boolean).map(String).filter(s=>s.includes('http')).slice(0,4))if(!allOut.some(o=>o.vin===vin&&o.image===im))allOut.push({vin,image:im});}}if(allOut.length===0){const cr=[...document.querySelectorAll('#one_search img')].map(i=>i.src).filter(s=>s.includes('images.cdn.manheim.com')).slice(0,800);const vs=[...document.documentElement.innerHTML.matchAll(/\b[A-HJ-NPR-Z0-9]{17}\b/g)].map(m=>m[0]);const seen=new Set();const uniq=vs.filter(v=>{if(seen.has(v))return false;seen.add(v);return true;});uniq.forEach((v,idx)=>{for(let k=0;k<3;k++){const im=cr[idx*3+k];if(im)allOut.push({vin:v,image:im});}});}const b=new Blob([JSON.stringify(allOut,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='manheim-con-fotos.json';a.click();alert('✅ '+allOut.length+' fotos ('+new Set(allOut.map(o=>o.vin)).size+' VINs) en '+page+' págs');window.fetch=origFetch;XMLHttpRequest.prototype.open=origOpen;XMLHttpRequest.prototype.send=origSend;})()`;
