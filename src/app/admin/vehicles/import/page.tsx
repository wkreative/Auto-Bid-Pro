...
const SCRIPT = `(async()=>{let last=0;for(let i=0;i<15;i++){window.scrollTo(0,document.body.scrollHeight);await new Promise(r=>setTimeout(r,1200));if(document.body.scrollHeight===last)break;last=document.body.scrollHeight;}window.scrollTo(0,0);await new Promise(r=>setTimeout(r,900));const imgs=[...document.querySelectorAll('#one_search img[src*="images.cdn.manheim.com"]')];const map=new Map();for(const img of imgs){let p=img.closest('[data-vin], [data-testid*="listing"], article, li, .listing, .vehicle-card')||img.parentElement;let vin=null;for(let d=0;d<6&&p;d++){const t=(p.innerText||'')+(p.dataset?JSON.stringify(p.dataset):'');const m=t.match(/\\b[A-HJ-NPR-Z0-9]{17}\\b/);if(m){vin=m[0];break;}const dv=p.getAttribute&&p.getAttribute('data-vin');if(dv&&/^[A-HJ-NPR-Z0-9]{17}$/.test(dv)){vin=dv;break;}const href=p.querySelector&&p.querySelector('a[href*="vin"]');if(href){const hm=href.href.match(/\\b[A-HJ-NPR-Z0-9]{17}\\b/);if(hm){vin=hm[0];break;}}p=p.parentElement;}if(!vin){const html=document.documentElement.innerHTML;const all=[...html.matchAll(/\\b[A-HJ-NPR-Z0-9]{17}\\b/g)].map(m=>m[0]);const idx=imgs.indexOf(img);vin=all[idx%all.length]||null;}if(!vin)continue;if(!map.has(vin))map.set(vin,[]);if(!map.get(vin).includes(img.src))map.get(vin).push(img.src);}let out=[];if(map.size>0){for(const[vin,images]of map)for(const image of images.slice(0,4))out.push({vin,image});}else{const cards=[...document.querySelectorAll('#one_search img')].map(i=>i.src).filter(s=>s.includes('images.cdn.manheim.com')).slice(0,800);const vins=[...document.documentElement.innerHTML.matchAll(/\\b[A-HJ-NPR-Z0-9]{17}\\b/g)].map(m=>m[0]);const seen=new Set();const uniq=vins.filter(v=>{if(seen.has(v))return false;seen.add(v);return true;});uniq.forEach((v,idx)=>{for(let k=0;k<3;k++){const im=cards[idx*3+k];if(im)out.push({vin:v,image:im});}});}const b=new Blob([JSON.stringify(out,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(b);a.download='manheim-con-fotos.json';a.click();alert('✅ '+out.length+' fotos capturadas ('+new Set(out.map(o=>o.vin)).size+' VINs)')})()`;
